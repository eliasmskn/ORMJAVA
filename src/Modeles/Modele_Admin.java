...
public class Modele_Admin {
	...
	public static String [] verifConnexion(String identifiant, String mdp)
	{
		String resultat [] = new String[3];
		String requete = "Select count(identifiant) as nb, nom, prenom"
				+ " from Admin where identifiant ='"+identifiant
				+"' and mdp ='"+mdp+"';";
	...
	public static LinkedList<Prof> selectall()
	{
		...
		Modele unModele = new Modele("127.0.0.1", "gestion_planning", "elias", "63523724");
		unModele.connexion();
		try{
			Statement unStat =  (Statement) unModele.getMaConnexion().createStatement();
			...
			while(unRes.next())
			{
				int id_proff = unRes.getInt("id_prof");
				String nom_p = unRes.getString("nom");
				String prenom_p = unRes.getString("prenom");
				String identifiant_p = unRes.getString("identifiant");
				String mdp_p = unRes.getString("mdp");
				Prof unP = new Prof(id_proff, nom_p, prenom_p, identifiant_p, mdp_p);
				uneListe.add(unP);
			}
			unStat.close();
		}
